...
Image::Image(int Width, int Height) 
{
    width = Width; 
    height = Height;
    pixels = std::vector<uint8_t>(static_cast<size_t>(width) * height * 3, 0);
}
...
void Image::setPixelColor(int x, int y, const Vec3f& color) 
{
...
{
    int stride = width * 3;
    return stbi_write_png(path.c_str(), width, height, 3, pixels.data(), stride) != 0;
}